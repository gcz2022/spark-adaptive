...
public class UnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
  ...
  void closeAndWriteOutput() throws IOException {
    ...
    sorter = null;
    final MapStatistics mapStats;
    final File output = shuffleBlockResolver.getDataFile(shuffleId, mapId);
    final File tmp = Utils.tempFileWith(output);
    try {
      try {
        mapStats = mergeSpills(spills, tmp);
      } finally {
      ...
      shuffleBlockResolver.writeIndexFileAndCommit(shuffleId, mapId, mapStats.lengths, tmp);
    } finally {
      if (tmp.exists() && !tmp.delete()) {
        logger.error("Error while deleting temp file {}", tmp.getAbsolutePath());
      }
    }
    mapStatus = MapStatus$.MODULE$.apply(blockManager.shuffleServerId(), mapStats.lengths, mapStats.records);
  }
   ...
   */
  private MapStatistics mergeSpills(SpillInfo[] spills, File outputFile) throws IOException {
    ...
    try {
      if (spills.length == 0) {
        ...
        return new MapStatistics(new long[partitioner.numPartitions()], new long[partitioner.numPartitions()]);
      } else if (spills.length == 1) {
        // Here, we don't need to perform any metrics updates because the bytes written to this
        // output file would have already been counted as shuffle bytes written.
        Files.move(spills[0].file, outputFile);
        return new MapStatistics(spills[0].partitionLengths, spills[0].partitionRecords);
      } else {
        final MapStatistics mapStats;
        ...
        if (fastMergeEnabled && fastMergeIsSupported) {
          ...
          if (transferToEnabled && !encryptionEnabled) {
            ...
            mapStats = mergeSpillsWithTransferTo(spills, outputFile);
          } else {
            logger.debug("Using fileStream-based fast merge");
            mapStats = mergeSpillsWithFileStream(spills, outputFile, null);
          }
        } else {
          logger.debug("Using slow merge");
          mapStats = mergeSpillsWithFileStream(spills, outputFile, compressionCodec);
        }
   ...
   * @return the partition lengths in the merged file.
   */
  private MapStatistics mergeSpillsWithFileStream(
      SpillInfo[] spills,
      File outputFile,
    ...
    return new MapStatistics(partitionLengths, partitionRecords);
  }
   ...
   * @return the partition lengths in the merged file.
   */
  private MapStatistics mergeSpillsWithTransferTo(SpillInfo[] spills, File outputFile) throws IOException {
    assert (spills.length >= 2);
    final int numPartitions = partitioner.numPartitions();
    final long[] partitionLengths = new long[numPartitions];
    final long[] partitionRecords = new long[numPartitions];
    ...
    try {
      ...
      for (int partition = 0; partition < numPartitions; partition++) {
        for (int i = 0; i < spills.length; i++) {
          ...
          partitionRecords[partition] += spills[i].partitionRecords[partition];
        }
      }
